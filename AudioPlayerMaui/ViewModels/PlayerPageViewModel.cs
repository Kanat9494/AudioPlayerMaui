...
public class PlayerPageViewModel : INotifyPropertyChanged
{
    ...
    #region Constructor
    public PlayerPageViewModel(IAudioPlayerService audioPlayerService, IRecordAudioService recordAudioService)
    {
        ...
        TestNavigate = new Command(ToSecondPage);
    }
    ...
    public ICommand TestNavigate { get; }
    ...
    {
        recordTimer = Microsoft.Maui.Controls.Application.Current.Dispatcher.CreateTimer();
    ...
    private async void ToSecondPage()
    {
        await Shell.Current.GoToAsync($"{nameof(SecondPage)}?{nameof(SecondPageViewModel.Test)}=test imasdflkje");
    }
    ...
    {
        if (playTimer == null)
            playTimer = Microsoft.Maui.Controls.Application.Current.Dispatcher.CreateTimer();
